...
public class MongoExample02 : MonoBehaviour
{
    private void Start()
    {
        ...
        filter["year"] = 2002;
        List<BsonDocument> data2002 = movieCollection.Find(filter).Limit(5).ToList();

        // 3-2. '필터 쿼리'를 사용한 방식
        FilterDefinition<BsonDocument> filter2 = Builders<BsonDocument>.Filter.Gt("year", 2002); // Gt: 초과
        List<BsonDocument> data20022 = movieCollection.Find(filter2).Limit(5).ToList();

        // 4. 논리 연산자(And, Or, Not)
        // if(1992 <= year && year <= 2002)
        var filterGte1992 = Builders<BsonDocument>.Filter.Gte("year", 1992); // Gte: 이상
        var filterLte2002 = Builders<BsonDocument>.Filter.Lte("year", 2002); // Lte: 이하
        var filterFinal = Builders<BsonDocument>.Filter.And("year", filterGte1992, filterLte2002);
